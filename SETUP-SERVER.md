...
# Apache Integration with "mod_wsgi" module:

Slackware includes Apache, but you need to build "mod_wsgi" module yourself.

## 1. Download and Modify SlackBuild Script

Use the SlackBuild script from [SlackBuilds.org](https://slackbuilds.org/repository/15.0/network/mod_wsgi/), but **update the version** to 5.0.2:
...
### Important: Force Correct Python Version

Edit the configure line in the script to:

```sh
./configure --with-python=/usr/bin/python3.9
```

This ensures mod\_wsgi uses Python 3.9 instead of defaulting to Python 2.7.

## 2. Enable mod\_wsgi in Apache
...
Add the following to `/etc/httpd/extra/pgadmin.conf` and include it in `httpd.conf`:
...
<VirtualHost *:80>
    ...
    ErrorLog "/var/log/httpd/error-pgadmin_log"
    CustomLog "/var/log/httpd/access-pgadmin_log" common
    # ScriptLog "/var/log/httpd/pgadmin-script_log"
...
For questions or refinements, edit this README.server accordingly or contact the maintainer.

I configure Apache log files as you saw; I will change the LogLevel to "warn" because "debug" writes a lot of stuff. But what is your take about my log files configuration?
